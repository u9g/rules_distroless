...
def passwd(name, entries, mode = "0644", time = "0.0", base = None, **kwargs):
    """
    ...
    Args:
        ...
        base: an image to add the entries to, as `useradd` would: an OCI image layout,
            such as `oci.pull` or `oci_image` makes, or a tar of a root filesystem.
            The file is then the base's /etc/passwd, from the newest layer that has it,
            followed by the entries, none of which may name a user the base has.
